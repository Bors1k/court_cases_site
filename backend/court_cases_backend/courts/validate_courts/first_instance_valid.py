...
def validate_fstinst_dates_of_court_hearing(court: CourtCases):
    ...
    if date_regex.findall(str(court.fstinst_dates_of_court_hearing)).__len__() != 0:
        ...
        if old_date is True and court.fstinst_date_of_dicision is None:
            ...
            task.notify_message = f"Не заполнена графа первой инстанции 'Дата вынесения решения(только дата)' в деле №{court.id}, куратора {court.user_id}"
            ...
            if court.fstinst_date_of_filing_in_court is not None or court.fstinst_date_of_receipt_of_judgment is not None:
                ...
                print('task created')
                task = get_task_for_validator(court=court,need_create=True, column_name=column_name)
                task.notify_message = f"Не заполнены графы первой инстанции 'Дата направления заявления в суд на выдачу судебного акта.' или 'Дата получения судебного решения' в деле №{court.id}, куратора {court.user_id}"
                task.date_of_notify = notify_date
        ...
        print('Создаем задачу')
        task = get_task_for_validator(court=court,need_create=True, column_name=column_name)
        task.notify_message = f"Не заполнена графа первой инстанции 'Даты судебных заседаний' в деле №{court.id}, куратора {court.user_id}"
...
def validate_fstinst_date_of_dicision(court: CourtCases):
    ...
    if court.fstinst_date_of_dicision is not None and str(court.fstinst_brief_operative_part).lower().__contains__('взыскать с') is False:
        print('Создаем задачу')
        task = get_task_for_validator(court=court, need_create=True, column_name=column_name)
        task.notify_message = f"Не заполнена графа первой инстанции 'Краткая резолютивная часть судебного акта' в деле №{court.id}, куратора {court.user_id}"
    ...
    and (court.fstinst_date_appeal_by_the_parties is None and court.fstinst_date_appeal_to_the_court is None):
        print('Создаем задачу')
        task = get_task_for_validator(court=court, need_create=True, column_name=column_name)
        task.notify_message = f"Не заполнены графы первой инстанции 'Дата направления апелляционной жалобы сторонам по делу' или 'Дата направления апелляционной жалобы в суд' в деле №{court.id}, куратора {court.user_id}"
...
def validate_fstinst_minfin_information(court: CourtCases):
    ...
    if court.fstinst_date_of_dicision is not None and str(court.fstinst_brief_operative_part).lower().__contains__('взыскать с'):
        if x_regex.findall(str(court.fstinst_minfin_information).lower()).__len__() == 0 and date_regex.findall(str(court.fstinst_minfin_information)).__len__() == 0:
            print('Создаем задачу')
            task = get_task_for_validator(court=court, need_create=True, column_name=column_name)
            task.notify_message = f"Не заполнена графа первой инстанции 'Информация о направлении справки по делу в ФК или Минфин' в деле №{court.id}, куратора {court.user_id}"
